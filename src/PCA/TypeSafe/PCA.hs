...
import PCA.Types
import PCA.TypeSafe.Types
import PCA.TypeSafe.Util
...
makePCATypeSafe desiredDim inputData_ =
  let (meanVec, covariance_) = meanCovSM inputData_
      ...
      eigenVecsf@(DimMatrix _) = getFirstColumns @d eigenVectors_
      finalData'@(DimMatrix _) = (transposeM eigenVecsf) `mulM` transposeM adjustInput' -- data in new eigenvectors space
      finalData_ = transposeM finalData'
      eigenVecsf' = transposeM eigenVecsf
      restoredDataWOMean@(DimMatrix _) = transposeM $ pinvSM eigenVecsf' `mulM` finalData'  -- restore to the original space
