...
import Universum hiding
  (All, Vector, head, map, sum, toList, trace, transpose, traverse,
  zipWith)
...
meanColumnWithNan
    ...
    => Matrix D a
    -> Vector D a
meanColumnWithNan matrix@(ADelayed (Z :. (_ :: Int) :. _ ) _) =
...
toMatrix arr desiredSize =
    ...
    where
        generator (Z :. _ :. cols) = linearIndex arr cols
